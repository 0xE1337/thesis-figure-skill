...
- **回路箭头**（如全局模型下发、反馈循环等）：必须指向**所有相关目标节点**，不能只指向其中一个。对于多个并列节点，推荐使用**总线分发模式**：从源节点出发画一条主干线到图的空白区域，主干线经过每个目标节点的对应 y 坐标时分叉一条短箭头进入节点。主干线转弯处使用 `rounded corners=6pt` 保持流畅
- **回路走线位置**：回路线必须走在**所有节点和标注之外的空白区域**。具体做法：计算节点边缘坐标，回路主干线至少偏移 0.7cm 以上，确保分叉箭头有足够可见长度（至少 0.5cm），不会贴着节点框
- **回路箭头标注**：回路线的标注文字应水平放置在主干线旁边的空白处（如底部横线段上方），不使用 rotate=90 竖排
- **跨层标注**：在两个分区框之间的空白处放置数据流标注文字，不要让标注与层标题重叠。标注与层标题之间至少保留 5pt 间距
- **虚线/回路不穿越节点**：所有虚线（异步连线、回路下发线等）必须绕行到节点外侧，不能穿过其他节点框或标注文字。具体做法：从源节点出发后先走到图的最外侧（留出足够偏移），再转向目标节点
...
- [ ] **回路主干线与节点边缘至少 0.7cm 间距**，分叉箭头有足够可见长度
- [ ] **箭头旁标注使用了 `tag` 样式**（`fill=white`），防止被交叉线遮挡
