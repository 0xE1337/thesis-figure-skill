...
- **三阶段自动化质检**（NEW）：编译前坐标验证（tikz-validator）→ 编译后重叠检测（pdf-overlap-checker）→ 可选路径规划（tikz-path-router），告别肉眼审查
- **PDF 精确重叠检测**（NEW）：基于 pdfplumber 解析 PDF 文字坐标（精度 0.1pt），自动检测文字重叠、文字溢出容器、线穿过文字、连线交叉
- **A* 路径规划器**（NEW）：输入节点位置和连线需求，自动输出避障正交路径坐标，解决密集连线区域的线交叉问题
- **设计野心体系**：最低设计门槛（≥15 视觉元素、≥3 级大小层次）、ASCII 布局草图、三轮自审（找 bug→找平庸→找排版）
- **防御性 TikZ 模板**：`shorten` 防箭头刺入、`-|`/`|-` 防拐弯贴框、coordinate 分叉防断连、background 层防遮挡
- **进化基线**：已验证最佳参数（间距/箭头/配色），标准只升不降
- **55+ 条经验记录**：34 批次的踩坑经验，自动复用

## v2.0 更新

- **数据可视化嵌入**：在架构图中嵌入热力图、柱状图、波形曲线、雷达图、散点图等数据可视化，一张图同时展示架构 + 数据
- **44 项审查规则**：从 33 条增加到 44 条，新增坐标轴标签防重叠、箭头精度、zone 边界完整性等
- **Skill 结构重构**：审查清单分离到独立文件，按需加载，节省 ~200 行 token
- **45 条经验记录**：25 批次 157 张图的踩坑经验，自动复用
