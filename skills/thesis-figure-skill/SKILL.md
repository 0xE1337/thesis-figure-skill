...
4. **一致性直觉**：人眼会自动寻找规律。如果两条线看起来应该平行（比如上传和下载），那它们就必须严格平行。如果三个框看起来应该等宽，那它们就必须严格等宽。**"看起来应该一样"的东西如果不一样，比"本来就不一样"的东西更刺眼。**

5. **线条的优雅感**：好的连线像水流——弯折干净利落（严格 90 度圆角），路径选择走最短最自然的那条。弯折处不是 90 度说明路径点没对齐；能直连的地方绕了弯说明出发/目标锚点选错了。每条线都应该让人感觉"这条路是显而易见的"。

6. **箭头的方向感**（最常被忽视的问题）：每条箭头必须在视觉上"流向"它逻辑上要去的方向。人看图时会直觉地感受到"这个箭头方向不对"——即使箭头最终连到了正确的目标。**核心问题是出发锚点和到达锚点的选择**：
   ...
   - **常见错误**：多条线从同一节点出发时，为了避免重叠而用了错误方向的锚点（比如目标在上方却从 `.east` 出发）。正确做法是用同一方向的不同锚点（`.north west` 和 `.north east`），或微调目标锚点（`yshift`），而不是换方向

7. **做减法的勇气**：编号圆、装饰标签、白色背景方块——每个元素都要问"它在传达什么信息？"如果答案是"纯装饰、没有信息量"，就去掉。少一个不需要的元素，比多一个好看但无用的元素更好。

**具体检查方向**（审美直觉的具体应用）：

1. **连线悬空**：线的两端都连在节点上吗？有没有"连到空气"的线？
2. **文字方向**：所有文字都能自然阅读吗？竖排从上到下？
3. **遮挡层次**：文字 > 框体 > 箭头线。如果箭头在文字上方，层次反了。
4. **微重叠**：两个元素之间有没有 1-2px 的轻微重叠？人眼对"差一点碰到"极其敏感——要么完全不碰（留足空气），要么明确重叠（如标签在线上），"差一点碰到"最难看。
5. **居中对齐**：标题居中了吗？底部元素居中了吗？
6. **字图比例**：图大字就应该大，图小字就可以小。
7. **时序图骨架**：生命线从上到下连续可见，是时序图的骨架。
8. **箭头方向感逐条审查**（最重要的审查项之一）：**沿着每条箭头，从起点到终点用眼睛"走"一遍**。问自己：这条箭头第一眼看起来在往哪个方向走？这个方向和数据/控制流的逻辑方向一致吗？如果一条"数据从下到上流"的箭头，视觉上看起来像是先往右/左绕了一圈——那就是锚点选错了。**这是最常见的被AI忽略但人一眼就能看出的问题。**
9. **辅助标注的指向自然性**：指向某个元素的辅助标签箭头不需要死板地指向元素的正中心——可以指向偏左/偏右/偏上的位置，只要读者能理解"这是在标注那个东西"就行。死板的正中心指向有时反而不自然（比如从左下方标签指向正上方元素的正底部，看起来很僵硬），偏移一点更像人手画的标注。
