...
25. **多源→多目标连线禁止交叉斜线**：当多个源节点（如"结果A""结果B"）需要连接到多个共享目标（如底部评估指标），**绝对禁止斜线交叉**——斜线交叉是最丑的连线模式。正确做法：每个源只连它最近的目标（左侧源连左侧目标、右侧源连右侧目标），或使用共享水平 bus（所有源先垂直下降到同一 y 高度的水平汇聚线，再从汇聚线分支到各目标）。**审查时看底部连线区域：有没有线交叉成 X 形？有就必须重新路由。**
