...
- [ ] **重叠遮挡**：是否有文字被连线、框体或其他文字遮挡？**图例框是否遮挡了公式框或其他内容？**
...
审查时不要逐条对照规则清单——而是**想象自己是第一次看到这张图的读者**。你的眼睛会自然地扫过整张图，注意力会被不自然的地方吸引。任何让你"觉得怪怪的"的地方都是问题，即使你说不出违反了哪条规则。具体方法：

1. **连线悬空检查**：每条连线的起点和终点是否都准确连接到了节点的锚点？有没有线"连到了空气"（起点或终点不在任何节点上）？
2. **文字方向检查**：图中所有文字（包括标签、标注、竖排侧栏）是否都能自然阅读？需要歪头或翻转才能读的文字必须修复。竖排文字必须从上到下阅读。
3. **遮挡优先级**：当箭头线和框体在同一区域时，想想这条线是否必要——如果线挡住了框里的重要文字，**优先保证文字可读**，线可以绕路或去掉。
4. **居中对齐**：标题是否在整图水平中心？底部元素（图例、结论框）是否居中？人眼对不居中非常敏感。
5. **字体大小与图的比例**：图越大，字体应该越大。如果整图宽度 > 15cm 但字号还是 \scriptsize，读者会看不清。
6. **时序图生命线**：生命线虚线不能被 combo 框、注释框等元素完全遮挡——生命线是时序图的骨架，必须从上到下连续可见。
