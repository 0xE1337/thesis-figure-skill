...
9. **辅助标注的指向自然性**：指向某个元素的辅助标签箭头不需要死板地指向元素的正中心——可以指向偏左/偏右/偏上的位置，只要读者能理解"这是在标注那个东西"就行。
...
11. **装饰元素方向正确性**：大括号、箭头、弧线等装饰元素有固有的方向性。大括号 `{` 开口朝右标注右侧内容，`}` 开口朝左标注左侧内容。如果方向搞反了（大括号开口背对内容），比没有还难看——直接去掉改用文字标注更好。
...
14. **弯折后线段长度检查**：每个弯折点之后，到下一个弯折点或箭头尖之间的线段是否 ≥ 0.8cm？如果弯折点紧挨着箭头（弯折处就是箭头尖），看起来像箭头断在了弯角——必须提前弯折。
...
19. **虚线类型区分**：反馈虚线和 zone 边框虚线在同一区域交叉时，读者分不清哪条是哪条。反馈线的 rail 必须在 zone 边框外侧 ≥ 1.0cm，确保两种虚线空间上完全分离。
20. **微斜线是最隐蔽的 bug**：两条线"看起来"平行但其实差了几度——这比明显的斜线更刺眼，因为人脑期望它们平行却发现不是。**每条非直连线都要验证**：路径中相邻两个坐标点是否共享 x 或 y？如果不共享就是隐式斜线。这个问题每轮测试都出现，说明光写规则不够——审查时必须**放大看每个弯折点**，问自己"这两段线真的是水平+垂直吗？"
