...
3. **遮挡层次**：文字 > 框体 > 箭头线。如果箭头在文字上方，层次反了。**zone 标题尤其容易被跨 zone 连线盖住**——因为 zone 标题在 zone 框的顶部，而跨 zone 的连线正好从上方进入这个区域。解决方法：zone 标题用独立 `\node` 绘制，放在所有 `\draw` 连线之后（TikZ 后画的在上层）。渲染后逐个 zone 标题检查：有没有连线穿过标题文字？
