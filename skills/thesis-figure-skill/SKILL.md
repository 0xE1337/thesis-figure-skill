...
33. **嵌入内容不能溢出容器框**（最像人的检查）：框内嵌入的可视化（热力图、柱状图、波形）及其标签，**每一个像素都必须在框的边界内**。这是人一眼就能看出但 AI 最容易忽略的问题——AI 计算坐标时只关注可视化本身，忘了标签、坐标轴文字、padding 也占空间。**代码层面验证方法**：计算框的 y 范围（center ± height/2），再计算内容的最底部 y（scope shift + 行数 × 行高 + 标签高度），确认内容底部 > 框底部 + 0.3cm padding。如果标签溢出了——要么上移内容，要么加大框高度。**渲染后检查**：逐个"框内有嵌入可视化"的框，放大看——可视化的标签/文字有没有跑到框线外面？底部/右侧有没有内容被框线截断？
