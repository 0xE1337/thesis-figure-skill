...
## 🔴 承载真实数字时的正确性铁律（借鉴 nature-figure QA，最优先）

bar-chart / heatmap / scatter / confusion-matrix / metrics-card 等承载**真实数字**（论文 benchmark、用户给的数据）时，hardcode 坐标极易和数字对不上——这是几何 checker 抓不到、只能靠语义自检的 bug：

1. **数字必须有出处**：来自论文 / 用户提供的数据，**禁止臆造、凑数、用"看起来合理"的假值**。拿不到真实值 → 写占位 `[TODO: 数值]` 并问用户，不要编。
2. **视觉编码必须 ∝ 数值**：柱高 ∝ 数值、热力图格深浅 ∝ 数值、散点坐标 = 真实数据点、混淆矩阵格 = 真实计数。**渲染后逐一核对**：最高的柱对应最大的数？两个相近值的柱高肉眼也相近？归一化基准对不对（别拿 0–100 画 80–95 的窄带，见 dynamic y-axis）？
3. **坐标 / 参数满足约束**：几何 / 数学示意（椭圆曲线、向量、相图）里给的坐标必须满足声明的方程——R3-38 教训（原仅限数学图）推广到**所有承载数字的嵌入图**。

**归属**：这是 ④.5 **语义检查**的一部分；mode C 的 `semantic` 镜头要"独立重算图里所有数字"正是为此。geometry checker（validator / overlap）对"数字画错但几何没重叠"完全失明。
