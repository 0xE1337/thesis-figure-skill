...
8. **Z-order（图层顺序）**：文字标签和模块框必须在连线/箭头的**上方**（更高图层）。在 draw.io XML 中，后定义的 mxCell 在上层。确保：**先定义 edge（连线），后定义 vertex（模块框和文字）**——这样框和文字会覆盖在箭头上方，而不是箭头穿过文字上方。如果连线穿过模块区域，文字应盖住连线而非被连线遮挡
